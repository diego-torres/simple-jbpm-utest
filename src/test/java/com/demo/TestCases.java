...
public class TestCases extends JbpmJUnitBaseTestCase {
  ...
  public void TestTimerExecuted() {
    ...
    assertNodeActive(processInstanceId, ksession, "Task");
    List<NodeInstanceLog> timerExecutedList = getLogService().findNodeInstances(processInstanceId).stream()
        .filter(n -> n.getType() == NodeInstanceLog.TYPE_EXIT && "5 min".equals(n.getNodeName()))
